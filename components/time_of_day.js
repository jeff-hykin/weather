...
export const backgroundColorGenerator = (color, {startTime=6, endTime=20, sunriseDuration=1.5}) => {
    const hour = new Date().getHours()
    let start, middle, end, angle, percent
    let angle = (90 * (hour-1 / 23))
    if (hour < startTime) {
        start = "rgba(53,51,87,1)"
        middle = "rgba(38,52,106,1)"
        end = "rgba(24,52,126,1)"
        percent = 50
    } else if (hour < startTime) {
        start = "rgba(38,52,106,1)"
        middle = "rgb(235, 127, 32)"
        end = "rgb(249, 237, 64)"
        percent = (1-((hour-startTime)/sunriseDuration))*100
    } else if (hour < 11) {
        start = "rgba(38,52,106,1)"
        middle = "rgba(24,52,126,1)"
        end = "rgba(0,213,255,1)"
        percent = (1-((hour-startTime)/(endTime-12)))*100
    }
    var start = "rgba(38,52,106,1)"
    var middle = "rgb(235, 127, 32)"
    var end = "rgb(249, 237, 64)"
    document.body.style.background = `linear-gradient(${angle}deg, ${start} 0%, ${middle} 47%, ${end} 100%)`
    return `linear-gradient(${angle}deg, ${start} 0%, ${middle} 47%, ${end} 100%)`
}
